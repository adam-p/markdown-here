...
Writing an email with code in it is pretty tedious. Writing Markdown with code in it is easy. I found myself writing an email in Markdown in the Github in-browser editor, then copying the preview into email. This is a pretty absurd workflow, so I decided to create a tool to write and render Markdown right in the email.
